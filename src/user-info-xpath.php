...
<?php
    if ($lFormSubmitted){
        try {
            ...
            $LogHandler->writeToLog("Received request to display user information for: " . $lUsername);
            ...
            if ($lXMLQueryResults){
                foreach ($lXMLQueryResults as $employee) {
                    // Original fields
                    echo "<p><strong>Username:</strong> " . ($lEncodeOutput ? $Encoder->encodeForHTML($employee->UserName) : $employee->UserName) . "</p>";
                    echo "<p><strong>Signature:</strong> " . ($lEncodeOutput ? $Encoder->encodeForHTML($employee->Signature) : $employee->Signature) . "</p>";
                    echo "<p><strong>Type:</strong> " . ($lEncodeOutput ? $Encoder->encodeForHTML($employee->Type) : $employee->Type) . "</p>";

                    // Adding new fields: ClientID and ClientSecret
                    echo "<p><strong>Client ID:</strong> " . ($lEncodeOutput ? $Encoder->encodeForHTML($employee->ClientID) : $employee->ClientID) . "</p>";
                    echo "<p><strong>Client Secret:</strong> " . ($lEncodeOutput ? $Encoder->encodeForHTML($employee->ClientSecret) : $employee->ClientSecret) . "</p>";
                }
            }else{
