...
class MySQLHandler {
    ...
    private function doCloseDatabaseConnection(){
        ...
        try{
            ...
            if (!$lResult) {
                throw (new Exception("Error executing query. Connection error: ".$this->mMySQLConnection->connect_errno." - ".$this->mMySQLConnection->connect_error." Error: ".$this->mMySQLConnection->errno." - ".$this->mMySQLConnection->error, $this->mMySQLConnection->errno));
            }// end if
    ...
    /**
     * Commit active transaction to database storage
     */
    public function commit() {
        if ($this->mMySQLConnection && method_exists($this->mMySQLConnection, 'commit')) {
            return $this->mMySQLConnection->commit();
        }
        return false;
    }
    ...
    public function rollback() {
        if ($this->mMySQLConnection && method_exists($this->mMySQLConnection, 'rollback')) {
            return $this->mMySQLConnection->rollback();
        }
        return false;
    }
